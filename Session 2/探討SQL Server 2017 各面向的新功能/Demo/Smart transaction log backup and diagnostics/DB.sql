...
--alter table [MeterMeasurement] set (SYSTEM_VERSIONING = OFF)
drop table if exists [MeterMeasurement]
--drop table if exists [dbo].[MeterMeasurementHistory]
go
CREATE TABLE [dbo].[MeterMeasurement] (
    [MeterID]          INT    identity                                     NOT NULL,
    [MeasurementInkWh] DECIMAL (9, 4)  default(1.0)                            NOT NULL,
    [PostalCode]       NVARCHAR (10)   default('123')                            NOT NULL,
    [MeasurementDate]  DATETIME2 (7)   default(sysdatetime())                            NOT NULL,
...
BACKUP DATABASE PowerConsumption TO disk = 'nul' WITH FORMAT,COMPRESSION
BACKUP LOG PowerConsumption TO disk = 'nul' WITH COMPRESSION
